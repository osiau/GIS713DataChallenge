...
# Basic models to consider all possible predictors
...
# Running second version of the models with insignificant predictors removed 
...
# Exploring forward stepwise as an approach
...
#summary(step_deaths)
```
We began by creating linear models that consider all numeric and categorical variables in the data set to predict covid cases and deaths per capita, respectively. We removed all of the numeric predictors that were not significant at the 5% level. 

# Finding best lm to predict covid cases per capita
```{r}
cases_v3 <- lm(formula = total_cases_pc ~ (vote_pct) + (beds_county) + (beds_state) + (hospitals_state), data=county_c_rem)
summary(cases_v3) # adj R^2 0.1099
plot(cases_v3$residuals~cases_v3$fitted.values)
abline(0,0)
qqPlot(cases_v3$resid)
hist(cases_v3$resid, breaks=14) 

cases_v4 <- lm(formula = total_cases_pc ~ sqrt(vote_pct) + 1/(beds_county) + sqrt(beds_state) + (hospitals_state), data=county_c_rem)
summary(cases_v4) # adj R^2 0.1688
plot(cases_v4$residuals~cases_v4$fitted.values)
abline(0,0)
qqPlot(cases_v4$resid)
hist(cases_v4$resid, breaks=14)

cases_v5 <- lm(formula = sqrt(total_cases_pc) ~ sqrt(vote_pct) + (beds_county)^2 + (beds_state) + sqrt(hospitals_state), data=county_c_rem)
summary(cases_v5) # adj R^2 0.2264
plot(cases_v5$residuals~cases_v5$fitted.values)
abline(0,0)
qqPlot(cases_v5$resid)
hist(cases_v5$resid, breaks=14)
```
...
```
We transformed individual predictor variables by exploring typical transformations for each variable, i.e. log(variable), sqrt(variable), (variable^2), (1/variable), etc. After trying each transformation for a given predictor, the transformation that most significantly improved the fit of the model (if any) was kept in the model. We determined an improvement of fit based on the plots (qqplot, histogram, residuals vs. fits, etc.) as well as the significance of each variable and adjusted R^2 value. This process was applied to all of the predictor variables and the response variable. Our best model to predict covid cases, with an adjusted R^2 of 0.2264, was: 
...
Also tried binary logistic, quasi-binomial, beta logistic, and beta loglog regressions 

# Exploring whether a county's color (blue vs non-blue, determined by % of votes) can be predicted based on unemployment rate and per-capita covid cases and deaths
```{r}
