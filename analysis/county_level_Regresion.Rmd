...
covid_red_blue <- glm(d2020268_pc ~ party, family = binomial (link="logit"), data = covid_cases3)
...
covid_cases3_Dem <- covid_cases3[party == "1"]
...
county_covid_Dem <- merge(county_shp, covid_cases3_Dem, by="countyFIPS")
    # as.Date("2020241", format="%Y%j") # which date is this?
spplot(county_covid_Dem, "d2020268_pc", xlim=c(-130, -65), ylim=c(25, 50), col=NA, col.regions=plasma(100))

# glm
covid_glm <- glm(d2020268_pc ~ voters_pc,  family = binomial (link="logit"), data = covid_cases3_Dem)
summary(covid_glm)

#lm
covid_lm <- lm(d2020268_pc ~ voters_pc, data = covid_cases3_Dem)
summary(covid_lm)
plot(covid_cases3_Dem$d2020268_pc ~ covid_cases3_Dem$voters_pc)
...
covid_cases3_Dem_removed <- covid_cases3_Dem[-c(125,327)]

# Plot again

# glm
covid_glm2 <- glm(d2020268_pc ~ voters_pc,  family = binomial (link="logit"), data = covid_cases3_Dem_removed)
summary(covid_glm2)

# lm
covid_lm2 <- lm(d2020268_pc ~ voters_pc, data = covid_cases3_Dem_removed)
summary(covid_lm2)
plot(covid_cases3_Dem_removed$d2020268_pc ~ covid_cases3_Dem_removed$voters_pc)
